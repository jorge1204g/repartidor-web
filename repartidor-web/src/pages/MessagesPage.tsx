  ...
  useEffect(() => {
    ...
    const initMessagesPage = async () => {
      try {
        ...
        const unsubscribe = MessageService.observeMessagesBetweenUsers(deliveryId, 'admin', async (receivedMessages) => {
          // Transformar los mensajes para que tengan el formato esperado por la UI
          const transformedMessages = receivedMessages.map(msg => ({
            ...msg,
            sender: msg.senderId === 'admin' || msg.senderId.startsWith('admin') ? 'Administrador' : msg.senderName,
            isOwn: msg.senderId === deliveryId
          }));
          setMessages(transformedMessages);
          
          // Contar mensajes no leídos antes de marcarlos como leídos
          const unreadBefore = await MessageService.countUnreadMessagesForUser(deliveryId);
          
          // Marcar todos los mensajes entrantes como leídos
          const incomingMessages = receivedMessages.filter(msg => 
            (msg.senderId === 'admin' || msg.senderId.startsWith('admin')) && !msg.isRead
          );
          for (const message of incomingMessages) {
            await MessageService.markMessageAsRead(message.id);
          }
          
          // Contar mensajes no leídos después de marcarlos como leídos
          const unreadAfter = await MessageService.countUnreadMessagesForUser(deliveryId);
          
          // Solo actualizar el contador si ha cambiado
          if (isMounted && unreadAfter !== unreadBefore) {
            setUnreadCount(unreadAfter);
          }
        });
        ...
        // Iniciar contador de mensajes no leídos
        ...
        const unreadCountInterval = setInterval(updateUnreadCount, 5000);
