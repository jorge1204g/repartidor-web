...
class AuthService {
  ...
  async loginWithId(deliveryId: string): Promise<AuthResponse> {
    try {
      ...
      // Registrar el usuario en el sistema de autenticación con rol
      await this.registerUserInAuthSystem(deliveryId, 'delivery');
  ...
  // Método para registrar al usuario en el sistema de autenticación con su rol
  private async registerUserInAuthSystem(userId: string, role: string): Promise<void> {
    try {
      ...
      console.error('Error registrando usuario en sistema de autenticación:', error);
    }
  }
