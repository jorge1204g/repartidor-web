  ...
  observeMessagesBetweenUsers(
    ...
    const unsubscribe = onValue(messagesRef, async (snapshot) => {
      if (snapshot.exists()) {
        ...
        // Obtener los mensajes anteriores para esta conversación
        const previousConversationMessages = this.previousMessages.get(conversationKey) || [];
        
        // Detectar nuevos mensajes (mensajes que no estaban antes)
        const newMessages = sortedMessages.filter(currentMsg => 
          !previousConversationMessages.some(prevMsg => prevMsg.id === currentMsg.id)
        );
        
        // Filtrar nuevos mensajes que fueron enviados por el otro usuario (no por el usuario actual)
        const newIncomingMessages = newMessages.filter(msg => msg.senderId !== userId1);
        
        if (newIncomingMessages.length > 0) {
          // Hay nuevos mensajes entrantes, reproducir sonido
          AudioNotificationService.playMessageReceivedSound();
        }
        
        // Actualizar los mensajes anteriores para esta conversación
        this.previousMessages.set(conversationKey, [...sortedMessages]);
